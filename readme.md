...
Modo Degradado (Fallback de Resiliencia): Arquitectura con tolerancia a fallos que intercepta las peticiones de red. Si el backend o PostGIS no responden **y** la SPA + assets + GeoJSON en `/public/geo/` están servidos, la interfaz conmuta a **operación degradada** con datos estáticos (p. ej. demos en Vercel). El fallback **sustituye la fuente de datos**, no promete "disponibilidad del 100%": si la CDN, la SPA o los shards estáticos fallan, la carga puede fallar con error honesto.
...
2. Filtrado Micro (Barrios y Corredores): A través de las pestañas inferiores, el usuario puede alternar la vista hacia Barrios o Corredores de Transporte (líneas de Subte A-H, Trenes Mitre/Sarmiento/Roca). Al seleccionar un corredor, el mapa dibuja automáticamente el área de influencia con radios de producto **500 / 1000 / 2000 m** (etiquetados en UI como 5 / 10 / 20 cuadras; conversión canónica `1 cuadra ≈ 100 m`).
...
```
>(Nota: Si no se configura `VITE_GEO_API_BASE` / Geo API, y la SPA sirve `/public/geo/`, la app entra en **modo degradado** con GeoJSON estático. Eso no garantiza disponibilidad total si fallan CDN/assets.)

4. Iniciar el servidor de desarrollo:
```bash
npm run dev
```
>Servicios locales (no son URLs equivalentes):
>- **Frontend (Vite):** http://localhost:5174
>- **Geo API (Node/PostGIS, dev):** http://localhost:3333 — también vía proxy Vite `/api/geo`
>- **Backend Go (API dominio):** http://localhost:8080 — p. ej. `PUT …/zona-de-vida` (requiere `npm run backend:api`)
>
>Para levantar el stack completo: `npm run stack` (PostGIS + Geo API :3333 + Vite :5174) y, en otra terminal, `npm run backend:api` si necesitás persistencia/handoff.
...
* Responsabilidad: Interceptar las solicitudes de red dirigidas al servidor geoespacial. Si PostGIS/API fallan **y** `/public/geo/` está disponible, conmuta a **modo degradado** con GeoJSON estático. No promete disponibilidad total si fallan SPA/CDN/assets.
...
* El desarrollo local se realiza sobre un entorno containerizado utilizando Docker Compose (docker-compose.yml), el cual levanta una instancia de PostgreSQL con la extensión PostGIS y las migraciones iniciales (db/init/).
  ...
  2.4.2.4. Estrategia de Despliegue en Modo Resiliente (Degraded Mode):
    ...
    * Si la API primaria (Go) o PostGIS no están alcanzables **y** el bundle estático + `/public/geo/` siguen disponibles, la app conmuta a **modo degradado** (operación limitada con GeoJSON estático). No es un SLA de disponibilidad total: sin SPA/CDN/assets, el fallback no aplica.
...
```mermaid
 erDiagram
    ...
    CORREDORES {
        ...
        geometry buffer_geom "Polygon (Área de influencia 500–2000 m; presets producto)"
    }
...
* **Atributos:**
  ...
  * `buffer_geom` (`GEOMETRY(Polygon, 4326)`, `NULLABLE`): Polígono calculado del área de influencia o distancia de caminabilidad a la traza (presets de producto: 500 / 1000 / 2000 m; 1 cuadra ≈ 100 m).
* **Relaciones:**
      ...
      parameters:
        ...
        - name: radio_m
          ...
          schema:
            ...
            enum: [500, 1000, 2000]
          description: >
            Radio de buffer en **metros** (unidad canónica).
            Presets de producto alineados a la UI: 500 / 1000 / 2000 m
            (etiquetas 5 / 10 / 20 cuadras; conversión visible 1 cuadra ≈ 100 m).
      responses:
        '200':
...
### Tareas Técnicas:
  ...
  3. Idempotencia de **productor**: clave natural (`event_id`) en outbox + estados `pending → published | parked`.
  
  4. Idempotencia de **consumidor** (obligatoria): el webhook/n8n (u otro receptor) debe deduplicar por `event_id` / Idempotency-Key. `event_id` en outbox **no** cubre la ventana en que el destino aceptó el evento y el relay aún no marcó `published`; el reintento puede reentregar. Semántica: **at-least-once en transporte / exactly-once efectivo en el receptor**.
  
  5. Desarrollar pruebas unitarias y de integración para el repositorio y handler (verificación de tabla outbox y rollback automático si falla la inserción del evento) + caso de reentrega documentado.
  
  6. Ejecutar smoke test manual: backend:api + relay → n8n/stdout; documentar en runbook operativo.
...
* Dado que el relay se encuentra activo y con un webhook configurado, when existe un evento de handoff pendiente, then el evento transiciona a published (o a parked con last_error detallado si el destino falla).

* Dado que un consumidor ya procesó `event_id=X`, when el relay reenvía el mismo envelope (falla entre ACK del webhook y `MarkPublished`), then el consumidor no duplica efectos de dominio (dedup por `event_id`); semántica: at-least-once en el cable / exactly-once efectivo en el receptor.
...
### Contrato espacial y de score (antes de implementar)

| Campo | Contrato |
|-------|----------|
| **`geom`** | `geography(Point, 4326)` — lon/lat WGS84; distancias en **metros geodésicos**. Inserción desde GeoJSON: `ST_SetSRID(ST_GeomFromGeoJSON($geojson), 4326)::geography`. No mezclar `geometry` (grados) con `ST_DWithin` en metros sin cast. |
| **Input de búsqueda** | (a) polígono GeoJSON de Zona de Vida → `ST_Intersects(p.geom, …::geography)`; (b) buffer opcional `radio_metros` → `ST_DWithin(p.geom, …, radio_metros)`; (c) `manzana_ids[]`. |
| **`score`** | `numeric(5,2)` nullable — score v0 (0–100); sin datos suficientes → `NULL` (nunca inventar). |
| **Unidad de caminata** | Canónica en API/SQL: **metros**. UI puede etiquetar cuadras con `1 cuadra ≈ 100 m`; presets ancla: **500 / 1000 / 2000 m**. |

### Tareas Técnicas:
1. Crear el script de migración `db/init/016_propiedades_inventario.sql` definiendo la tabla `propiedades` (`id`, `precio`, `ambientes`, `amenities jsonb`, `geom geography(Point,4326) NOT NULL`, `score numeric(5,2)`, `barrio_id`, `activo`).

2. Definir un índice espacial GIST sobre `geom` y un índice B-tree sobre `(activo, precio, ambientes)`.

3. Crear el script de datos iniciales (seed/fixture) con al menos 20 propiedades en Belgrano, Palermo y Núñez, insertando `geom` con casts explícitos.

4. Escribir la función SQL de búsqueda por zona con firma documentada (`$zona_geojson` / `$radio_metros` / casts) que retorne propiedades ordenadas por distancia/`score`.

5. Actualizar `docs/3-modelo-de-datos.md` y OpenAPI (`radio_metros` con `minimum`/`maximum` alineados a presets de producto).
...
* [x] Idempotencia del relay (productor) comprobada con duplicación de payloads.

* [ ] Idempotencia del **consumidor** documentada/exigida: dedup por `event_id` ante reentrega (at-least-once / exactly-once efectivo).
...
Visualización de la red subterránea y buffer de cobertura en **metros** (preset típico **1000 m** ≈ 10 cuadras; `CUADRA_METROS=100`).
...
<img width="1024" height="574" alt="Paso 1 - Selección por Comunas" src="https://github.com/user-attachments/assets/e93edc77-5728-4161-9570-460cb8acf8ec" />
...
<img width="1920" height="1080" alt="Paso 2 - Comuna 13 identificada" src="https://github.com/user-attachments/assets/5ef6ad9d-6005-4696-8cb2-22bd8fbf26d4" />
...
<img width="1920" height="1080" alt="Paso 3 - Barrio Belgrano seleccionado" src="https://github.com/user-attachments/assets/08be71ad-2d33-436c-aa1d-cd09492682d7" />
...
<img width="1024" height="576" alt="Paso 4 - Carga de la capa de manzanas en Belgrano" src="https://github.com/user-attachments/assets/02164e11-2aca-42ce-a1d6-4c64ce7d23e6" /><img width="1024" height="576" alt="image" src="https://github.com/user-attachments/assets/eeb50bc2-6087-4f67-bf56-192b4fd94c02" />
...
<img width="1024" height="576" alt="Paso 5 - Dibujo del polígono en el corredor Cramer - Tren" src="https://github.com/user-attachments/assets/99447dec-f5bb-4d29-93e0-e7538a71506c" />
...
<img width="1024" height="576" alt="Paso 6 - Detección automática de barrios colindantes" src="https://github.com/user-attachments/assets/ccdeb18a-0a5c-4e0a-8a52-66a73fa67f2e" />
...
<img width="1024" height="576" alt="Paso 7 - Menú emergente del asistente inteligente" src="https://github.com/user-attachments/assets/5358c9b6-7cc5-43b0-b34c-44ad30d319af" />
...
<img width="1024" height="576" alt="Paso 8 - Confirmación e integración de 122 manzanas" src="https://github.com/user-attachments/assets/c5bfd898-0ab9-4b5d-b663-c96b4721575a" />
...
<img width="1920" height="1080" alt="Paso 9 - Resumen y confirmación de la Zona de Vida" src="https://github.com/user-attachments/assets/b5d71b9e-7696-4dd1-83de-da3621a03813" />
...
<img width="1920" height="1080" alt="Paso 1 - Busqueda por Voz e Identificacion de Estacion Ancla" src="https://github.com/user-attachments/assets/7b60dc1d-0cb2-4be0-aec8-0e84f3ddd74e" />

2. Configuración de caminabilidad e indicadores: Seleccionamos el rango de caminabilidad cómodo (**1000 m** ≈ 10 cuadras; unidad canónica: metros) y la app calcula al instante los servicios del entorno: 0 Salud, 2 Comisaría, 70 Educación y 54 Cultura.<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/b14320a0-b376-41a8-beb5-fed61df98f80" />
...
5. Renderizado catastral por Manzanas (Capa Leaflet/PostGIS): Al conmutar a la puerta E. Manzanas, el motor geoespacial calcula el buffer de **1000 m** (≈ 10 cuadras) alrededor de ambas estaciones, pintando en verde las 1.985 manzanas contenidas bajo el criterio de caminabilidad seleccionado.<img width="1024" height="576" alt="image" src="https://github.com/user-attachments/assets/8cee9232-770d-430d-b98c-e7b7b537c19f" />
...
<img width="960" height="539" alt="Paso 8 - Tareas y responsabilidades asignadas al Propietario" src="https://github.com/user-attachments/assets/d847019c-bf71-4e88-beec-931b5dfc9aca" />
